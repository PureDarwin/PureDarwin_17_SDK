...
#define IOHIDLog(fmt, ...)        /* We can't use os_log* on PureDarwin, as    */
#define IOHIDLogError(fmt, ...)   /* the library that implements it (/usr/lib/ */
#define IOHIDLogDebug(fmt, ...)   /* system/libsystem_trace.dylib (part of the */
                                  /* libtrace project) is not open source.     */
...
#if defined(__BUILDING_PUREDARWIN__)
CF_EXPORT void* _Nullable _IOHIDLog(void);
CF_EXPORT void* _Nullable _IOHIDLogCategory(IOHIDLogCategory category);
#else /* !defined(__BUILDING_PUREDARWIN__) */
CF_EXPORT
